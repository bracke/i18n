...
This directory records the deterministic CLDR-derived data subset the generator
produces from the pinned upstream. It is emitted to two places:

- **`share/i18n` data files** (`formats.i18ndata` plus `units/`, `currency/`, and
  `zones/` shards) carry the per-locale formatting data — number/list separators,
  month/weekday/quarter/day-period names, date/relative patterns, currency and
  unit display names, timezone exemplar cities — served on the fly by
  `I18N.Locale_Data` (see `RUNTIME_DATA_FILES.md`).
- **`src/i18n-cldr_data.adb`** carries the structural remainder with no
  on-the-fly equivalent: timezone transition rules, `-u-nu-` numbering-system
  digit sets, plural-rule interpreters, calendar week rules, and the handful of
  scalar tables not yet moved to files (decimal/group separators, number affixes,
  time-style patterns, era names, zone display/short names).

Both are generated by `cldr/src/generate_cldr_data.adb` from the same subset.
